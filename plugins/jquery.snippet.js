			...
			if(node == "pre"){
				...
				if(!o.parent().hasClass("snippet-wrap")){
					...
					if(defaults.showNum){
						...
						newhtml="<ol class='snippet-num' start='" + defaults.startLineNum + "'><li>"+newhtml+"</li></ol>";
					...
					if(defaults.box!=""){
						...
						for(var i=0;i<boxNums.length;i++){
							var boxNum = boxNums[i];
								if(boxNum.indexOf('-')==-1){
									boxNum = parseFloat(boxNum)-defaults.startLineNum;
									o.find("li").eq(boxNum).addClass('box').prepend(spacer);
								} else {
									var numStart = parseFloat(boxNum.split('-')[0])-defaults.startLineNum;
									var numEnd = parseFloat(boxNum.split('-')[1])-defaults.startLineNum;
									if(numStart<numEnd){
				...
				// if node IS an existing Snippet...
					...
					if(defaults.showNum){
						...
						if(list.hasClass("snippet-no-num")){
							list.wrap("<ol class='snippet-num' start='" + defaults.startLineNum + "'></ol>");
